						...
						<div class="row grid-gallery">
							<c:if test="${!empty data }">
								...
								<span class="image fit img-panel">	
									<a href="picboarddetail.do?cont_id=${tmp.cont_id}" class="image">
										<img src="${tmp.pic}" alt="" />
									</a>
									<h5>
										<a class="icon"	href="picboarddetail.do?cont_id=${tmp.cont_id }">${tmp.content_title}</a>
									</h5>
									<ul class="alt">
				...
				<ul class="pagination">
					<c:choose>
						<c:when test="${startPageNum ne 1 }">
							<li>
								<a href="picboardlist.do?pageNum=${startPageNum-1 }">&laquo;</a>
							</li>
						</c:when>
						<c:otherwise>
							<li class="disabled">
					...
					<c:forEach var="i" begin="${startPageNum }" end="${endPageNum }">
						<c:choose>
							<c:when test="${i eq pageNum }">
								<li class="active">
									<a href="picboardlist.do?pageNum=${i }">${i }</a>
								</li>
							</c:when>
							<c:otherwise>
								<li>
					...
					<c:choose>
						<c:when test="${endPageNum lt totalPageCount }">
							<li>
								<a href="picboardlist.do?pageNum=${endPageNum+1 }">&raquo;</a>
							</li>
						</c:when>
						<c:otherwise>
							<li class="disabled">
